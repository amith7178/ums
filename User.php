...
class User extends Dbconfig {	
    ...
    public function __construct(){
        if(!$this->dbConnect){ 		
            ...
            if($conn->connect_error){
                die("Error - MySQL Connection Failure: " . $conn->connect_error);
            } else{
                $this->dbConnect = $conn;
            }
        }
	...
	private function getData($sqlQuery) {
		...
		if(!$result){
			die('Error - SQL query: '. mysqli_error());
		}
		$data= array();
	...
	public function register(){		
		...
		if(!empty($_POST["register"]) && $_POST["email"] !='') {
			...
			if($isUserExist) {
				...
				if($userSaved) {				
					$link = "<a href='http://localhost/user-management-system/verify.php?authtoken=".$authtoken."'>Verify Email</a>";			
					$toEmail = $_POST["email"];
					$subject = "Email Verification";
					$msg = "Hi, Use this ".$link." to verify email to complete registration.";
					$msg = wordwrap($msg,70);
					$headers = "From: admin@ums.com";
					if(mail($toEmail, $subject, $msg, $headers)) {
						$message = "Verification email has been sent. Please check and verify to complete registration.";
					}
				} else {
					$message = "Error - User register request failed.";
				}
			}
		}
		return $message;
	...
	public function resetPassword(){
		...
		if($_POST['email'] == '') {
			...
			if($numRows) {			
				...
				$link="<a href='https://localhost/user-management-system/reset_password.php?authtoken=".$authtoken."'>Reset Password</a>";				
				$toEmail = $user['email'];
				$subject = "Password Reset";
				$msg = "Hi, use this ".$link." to reset your password.";
				$msg = wordwrap($msg,70);
				$headers = "From: admin@ums.com";
				if(mail($toEmail, $subject, $msg, $headers)) {
					$message =  "Password reset link has been sent. Please check your email to reset password.";
				}				
			} else {
